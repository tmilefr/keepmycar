			...
			switch($def_menu->type){
				case 'top':
					foreach($def_menu->items AS $element){
						if ($this->CI->acl->hasAccess(strtolower($element->url)) OR $element->noright){
							$have_right = true; 
							$menu .= '<li class="nav-item"><a class="nav-link" href="'.(($element->noright) ? $element->url:base_url($element->url)).'"><span class="oi '.$element->icon.'"></span> '.Lang($element->name).'</a></li>';	
						}
					}					
				break;
				case 'li':
				...
				case 'dropdown':
					if (count($def_menu->items)){
						...
						$menu .= '<a class="nav-link dropdown-toggle" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false"><span class="oi '.$def_menu->icon.'"></span> <span class="mr-2 d-none d-lg-inline text-gray-600 small">'.$def_menu->name.'<span></a>';
