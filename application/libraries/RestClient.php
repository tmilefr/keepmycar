...
class RestClient implements Iterator, ArrayAccess {
	...
	protected $streamVerboseHandle = NULL;
	protected $verboseLog = NULL;
    ...
    public function __construct(array $options=[]){
        $default_options = [
            ...
            'user_agent' => "PHP RestClient/0.1.9.nl", 
            'base_url' => NULL, 
            'format' => NULL, 
            'format_regex' => "/(\w+)\/(\w+)(;[.+])?/",
            'decoders' => [
    ...
    public function execute(string $url, string $method='GET', $parameters=[], array $headers=[]) : RestClient {
		...
		if ($this->_debug){
			$this->streamVerboseHandle = fopen('php://temp', 'w+');
			curl_setopt($client->handle, CURLOPT_STDERR, $this->streamVerboseHandle);
        ...
        $this->http_code = curl_getinfo($client->handle, CURLINFO_HTTP_CODE);
        ...
        if ($this->_debug){
			rewind($this->streamVerboseHandle);
			$this->verboseLog[$url][] = stream_get_contents($this->streamVerboseHandle);
		}
    ...
    public function __destruct(){
		if ($this->_debug == TRUE){
			echo debug($this->verboseLog,__LINE__);
		}
	}
